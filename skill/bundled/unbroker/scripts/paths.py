...
All per-subject data lives under PDD_DATA_DIR (default: $DULUS_HOME/unbroker),
...
def hermes_home() -> Path:
    """Dulus home dir (kept name for upstream diff-ability)."""
    return Path(
        os.environ.get("DULUS_HOME")
        or os.environ.get("HERMES_HOME")  # upstream compat
        or (Path.home() / ".dulus")
    )
