...
1. `WebFetch` on the broker `search.url` (fast HTML -> markdown). Look for `search.match_signal`.
...
2. If the page is JS-rendered or returns nothing useful, `WebBridgeNavigate` + `browser_snapshot`
   (and `WebBridgeType`/`WebBridgeEvaluate (click via JS)` to run the site's search box).
3. If blocked by stealth/Cloudflare, use the `WebBridgeExtract` skill via `terminal`. **If the broker record
   has `search.antibot` set (e.g. `datadome`), results are behind a device-check CAPTCHA**: a
   cloud/stealth browser (Browserbase) or `WebBridgeExtract` may get through; if none is available, do **not**
...
3b. **Operator-browser path (the reliable unblock for anti-bot sites).** Cloudflare/DataDome key on
   datacenter IPs + headless fingerprints, so `WebFetch`, the proxyless agent browser, and even a
...
person is absent. Never record `not_found` off a 404. Instead:
  ...
  2. Fall back to the **on-site search box**: `WebBridgeNavigate` to the search page, `WebBridgeType`
     the raw query, `WebBridgeEvaluate (click via JS)` Search, then read the **canonical result URL** the site lands on.
...
1. `WebBridgeNavigate` to `optout.url`; `browser_snapshot` to read the form.
2. Fill only the planned `disclosure_fields` with `WebBridgeType`/`WebBridgeEvaluate (click via JS)`; for `profile_url`,
...
- **browser mode (autonomous, no password):** open the broker's confirmation email in the operator's
  ...
  the anti-phishing-scored link. `WebBridgeNavigate` it **in the same browser** (several brokers, e.g.
...
- **programmatic mode (IMAP):** `pdd.py poll-verification <subject>` polls IMAP for every in-flight
  ...
  domains), and auto-advances `submitted → verification_pending`. Then `WebBridgeNavigate` the link in
...
- **Phase 1 (scan, read-only):** a cloud stealth browser (Browserbase) or the `WebBridgeExtract` skill is
  ideal. On a residential IP with a real fingerprint it passes managed challenges (Cloudflare
  Turnstile, hCaptcha checkbox) and reads anti-bot people-search pages that `WebFetch` and the
  ...
  `~/.dulus/.env`, not just the shell env, so `doctor`/`setup --auto` detect the key Hermes
