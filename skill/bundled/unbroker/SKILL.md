...
metadata:
  hermes:
    ...
    category: security
    related_skills: [google-workspace, agentmail, himalaya, WebBridgeExtract, osint-investigation]
...
form-driving with native tools: `WebFetch` and `WebBridgeNavigate` for searching and web forms, and
`Reminder (or a scheduled background task)` for recurring re-scans.
...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `~/.dulus/.env`** so keys
  ...
  - The `WebBridgeExtract` skill for stealth/Cloudflare-protected pages.

## How to Run

Run everything through the `Bash` tool. From this skill's directory:

```bash
PDD="python3 scripts/pdd.py"
```

The engine stores data under `$PDD_DATA_DIR` (default `$DULUS_HOME/unbroker`), written
...
| `$PDD fanout <subject> [--priority crucial] [--size 5]` | Batch brokers into parallel `Agent` subagents (auto for large runs; batches of 5 - 8+ time out) |
...
- **Phase 1 - DISCOVER (read-only, parallel, idempotent).** Crawl *every* broker first and record a
  ...
  what unlocks cluster dedup and prioritization below. **Default: the parent drives `WebFetch`
  probes directly** - most people-search sites render name/phone/address results as static HTML that
  `WebFetch` reads in seconds. Escalate to `browser_*` only for the few JS-only sites, and to
  `Agent` subagents only for genuinely *reasoning*-heavy work (large-scale namesake/relative
  ...
  heavy; the ledger writes that survived came at 10x the cost of parent `WebFetch`. A `blocked`
...
4. **Scanning (when `next` says so).** For `fanout_scan`: run `$PDD fanout <subject>` and **spawn one
   `Agent` subagent per `batch`, in parallel, passing that batch's ready-made `brief`** - do
   ...
   ladder (`WebFetch` → `site:` probe → `WebBridgeNavigate` → `WebBridgeExtract`), a 404 is INCONCLUSIVE
...
5. **Opt-outs (when `next` says so).** Actions come pre-ordered parents-first with `steps` from each
   ...
   - **web_form** → drive `optout_url` with `WebBridgeNavigate`/`WebBridgeType`/`WebBridgeEvaluate (click via JS)`, submit
...
8. **Schedule the next wake-up.** `next` returns `next_wake_at` (earliest due re-check). Create ONE
   `Reminder (or a scheduled background task)` that re-runs this skill's loop for the subject (a prompt like: *"run the
...
- **Dossiers are plaintext by default** (JSON, `0600` under `DULUS_HOME`). For at-rest encryption run
  ...
  a full-`DULUS_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
